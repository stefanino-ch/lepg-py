c***************************************************************
c      LE PARAGLIDING v 3.15 "Canigó"
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
       lepv="3.15"           ! Version
       lepd="2021-01-17"      ! Date
       lepc='"Canigo"'           ! Code name
       lepuser="GENERAL"  
       ich=0
...
c       psep=1970.*xkf+seppix*float(i)
c       psey=400.*xkf
...
c      Control if cell is closed
c       if(int(rib(i,14)).eq.0.and.int(rib(i+1,14)).eq.0) then
c       psey=1291.*xkf
c       ncontrol=0
c       end if
...
c       do j=np(i,2)+ncontrol*(np(i,3)-1),np(i,1)-1
