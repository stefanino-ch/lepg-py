c***************************************************************
c      LE PARAGLIDING v 3.10 "Pirineus"
c      Pere Casellas 2010-2020
...
c       real*8 xlu1,xlv1,xlu2,xlv2
       ...
       character*50 wname, bname, nomair(100), lyname(50)
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
       lepv="3.10"           ! Version
       lepd="2020-05-02"      ! Date
       lepc='"Pirineus"'      ! Code name
       lepuser="GENERAL"  
       ich=0
...
c      Customized header
       ...
       write (*,*)
       write (*,'(A14,1x,A4,1x,A7,1x,A50)') " LEparagliding",lepv,
     + "version",lepc
       write (*,'(1x,A10)') lepd
       write (*,*)
       write (*,*) "Pere Casellas"
...
c      Read data for nose rods type 1
       if (k21d.eq.1) then
       ...
       read (22,*) xextra(ng,1),xextra(ng,2),xextra(ng,3),xextra(ng,4) !x1i,x1f,y1,exp1 
       read (22,*) xintra(ng,1),xintra(ng,2),xintra(ng,3),xintra(ng,4) !x2i,x2f,y2,exp2
       read (22,*) sjo(ng,1),sjo(ng,2),sjo(ng,3),sjo(ng,4)
       end do
       
       end if ! Type 1

c      Define jonc group rib(i,166)
       do i=1,nribss
       rib(i,166)=0.
       end do
       do ng=1,ng21
...
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
       ...
       backspace(22)
       read (22,*) j, rib(i,165),csi(i-1,19),csi(i-1,20)
       end if
...
c       do i=1,nribss
c       write (*,*) "number 3D group", i, int(rib(i,169))
c       end do
...
c       seppix=60.*xwf
       seppix=(amplemig*1.7+(0.20d0*xupp))*amplepix
...
c     y separation for panels parts
      ...
      ysautt=(rib(1,5)+rib(nribss-2,5))*0.5d0*xwf*0.2d0
...
c      Ribs
       ...
       call itxt(rib(i,2)-110.,rib(i,3)-15,10.0d0,0.0d0,i,7)
       call itxt(-rib(i,2)-110.,rib(i,3)-15,10.0d0,0.0d0,i,7)
...
c      Draw especial vents CASE 1
       ...
       call line(rib(i-1,2),rib(i-1,3)+rib(i-1,5)*rib(i,12)/100.,
     + rib(i,2),rib(i,3)+rib(i,5)*rib(i,12)/100.,6)
       call line(-rib(i-1,2),rib(i-1,3)+rib(i-1,5)*rib(i,12)/100.,
     + -rib(i,2),rib(i,3)+rib(i,5)*rib(i,12)/100.,6)
       end if
...
c      Draw especial vents CASE 0
     ...
     + rib(i,2),rib(i,3)+rib(i,5)*rib(i,12)/100.,6)
       call line(-rib(i-1,2),rib(i-1,3)+rib(i-1,5)*rib(i,12)/100.,
     + -rib(i,2),rib(i,3)+rib(i,5)*rib(i,12)/100.,6)
       call line(rib(i-1,2),rib(i-1,3)+rib(i-1,5)*rib(i,11)/100.,
     + rib(i,2),rib(i,3)+rib(i,5)*rib(i,11)/100.,6)
       call line(-rib(i-1,2),rib(i-1,3)+rib(i-1,5)*rib(i,11)/100.,
     + -rib(i,2),rib(i,3)+rib(i,5)*rib(i,11)/100.,6)
       end if
...
c       write (*,*) i, "nose ",np(i,6),u(i,np(i,6),2),v(i,np(i,6),2)
...
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
       ...
       call itxt(sepx-130.+2530.*xkf,sepy,10.0d0,0.0d0,i,7)
...
c       write (*,*) "Rib jonc ",i,rib(i,166)
       if (rib(i,166).ne.0) then
c       write (*,*) "www ",i
...
c      Box (1,4)
       ...
       call romano(i,sepx+0.89d0*rib(i,5),sepy-1.0d0,0.0d0,1.0d0,4)
...
c      Dibuixa marques romanes AD
       call romano(i,psep+5.,psey+0.5,0.0d0,0.4d0,1)
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
...
c      Draw romano in last panel
       ...
       call romano(i+1,psep+5.,psey+0.5,0.0d0,0.4d0,1)
       end if
...
c      Cal definiar abans als vectors u(i,j,11) v u(i,j,12) v
       call extpoints(i,uf,vf,npo,xupp,xupple,xuppte,1)
       call dpanelc(i,uf,vf,npo,psep,psey)
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccc
       ...
       psep=1970.*xkf+seppix*float(i)
       psey=500.*xkf-890.95*2.0*xkf ! remove 890.95 to set BOX (1,3)
...
c      Print equidistant points
     ...
     + xfinl,xfinr,xmark,llarl,llarr,psep,psey,xcir,xdes,xkf)
       ...
       call dpanelcc(i,uf,vf,npo,psep,psey,2)
       call dpanelcc(i,uf,vf,npo,psep+2520.*xkf,psey,6)
       call arc3p(i,npo,uf,vf,zinf(i+1,6,1),1,psep,psey,1,xupp)
       call arc3p(i,npo,uf,vf,zinf(i+1,6,1),1,psep+2520.*xkf,psey,
     + 2,xupp)
...
c      Print panels without vents       
       ...
       call arc3p(i,1,uf,vf,zinf(i+1,6,1),-1,psep,psey+ysaut,1,xupp)
       call arc3p(i,1,uf,vf,zinf(i+1,6,1),-1,psep+2520.*xkf,psey+ysaut,
     + 2,xupp)
...
c      Print equidistant points
     ...
     + xfinl,xfinr,xmark,llarl,llarr,psep,psey,xcir,xdes,xkf)
       ...
       call dpanelcc(i,uf,vf,npo,psep+2520.*xkf,psey,6)
       call arc3p(i,npo,uf,vf,zinf(i+1,6,1),1,psep,psey,1,xupp)
       call arc3p(i,npo,uf,vf,zinf(i+1,6,1),1,psep+2520.*xkf,psey,2,
     + xupp)
...
c       write (*,*) "X ",i,xinil,xinir
     ...
     + xfinl,xfinr,xmark,llarl,llarr,psep,psey+ysaut,xcir,xdes,xkf)
       ...
       call arc3p(i,1,uf,vf,zinf(i+1,6,1),-1,psep,psey+ysaut,1,xupp)
       call arc3p(i,1,uf,vf,zinf(i+1,6,1),-1,psep+2520.*xkf,psey+
     + ysaut,2,xupp)

       call arc3p(i,npo,uf,vf,zinf(i+1,6,2),1,psep,psey+ysaut,1,xupp)
       call arc3p(i,npo,uf,vf,zinf(i+1,6,2),1,psep+2520.*xkf,psey+
     + ysaut,2,xupp)
...
c      Print panels without vents       
       ...
       call arc3p(i,1,uf,vf,zinf(i+1,6,2),-1,psep,psey+ysaut,1,xupp)
       call arc3p(i,1,uf,vf,zinf(i+1,6,2),-1,psep+2520.*xkf,psey+
     + ysaut,2,xupp)
...
c      Print panels with vents       
       ...
       call dpanelcc(i,uf,vf,npo,psep+2520.*xkf,psey+ysaut,5)
       call arc3p(i,1,uf,vf,zinf(i+1,6,2),-1,psep,psey+ysaut,1,xupp)
       call arc3p(i,1,uf,vf,zinf(i+1,6,2),-1,psep+2520.*xkf,psey+ysaut,
     + 2,xupp)
...
c      Avoid central panel if thickness is 0
       ...
       psep=1970.*xkf+seppix*float(i)
       psey=500.*xkf-890.95*1.0*xkf ! remove 890.95 to set BOX (1,3)
...
c       if (k26d.eq.0.or.rib(i+1,165).ge.0) then
       ...
       call dpanelcc(i,uf,vf,npo,psep,psey+ysaut,4)
       call dpanelcc(i,uf,vf,npo,psep+2520.*xkf,psey+ysaut,8)
       call arc3p(i,1,uf,vf,zinf(i+1,6,4),-1,psep,psey+ysaut,1,xlow)
       call arc3p(i,1,uf,vf,zinf(i+1,6,4),-1,psep+2520.*xkf,psey+
     + ysaut,2,xlow)
c       end if
...
c      Print without vents
       ...
       call dpanelcc(i,uf,vf,npo,psep,psey,2)
       call dpanelcc(i,uf,vf,npo,psep+2520.*xkf,psey,6)
       call arc3p(i,npo,uf,vf,zinf(i+1,6,4),1,psep,psey,1,xlow)
       call arc3p(i,npo,uf,vf,zinf(i+1,6,4),1,psep+2520.*xkf,psey,2,
     + xlow)
       end if
...
c      xlll, ylll computed above       
     ...
     + -xv+psey-0.7*dcos(alprom)-5.*dsin(alprom),pi-alprom,0.4d0,4)
...
c      Airfoil anchor (u,v,w) espace coordinates
       u(i,j,18)=u(i,j,17)
       v(i,j,18)=v(i,j,17)*dcos(rib(i,9)*pi/180.0d0)
       w(i,j,18)=-v(i,j,17)*dsin(rib(i,9)*pi/180.0d0)
...
c      Customized header
       ...
       write (23,'(A31,1x,A4,1x,A7,1x,A50)') 
     + " LABORATORI D'ENVOL PARAGLIDING",lepv,"version",lepc
       write (23,*) "by Pere Casellas 2010-2020"
...
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
       ...
       x1=0.
       write (23,*)
       write (23,*) "8. NYLON RODS (JONCS) TYPE 1"
       write (23,*) "-------------------------------------------"
       do ng=1,ng21
       do i=ngo(ng,2),ngo(ng,3)
       x1=x1+rib(i,167)
       write (23,'(A5,I3,2x,F7.1)') "Jonc ",i,rib(i,167)
       end do
       end do
       write (23,'(A,F8.1)') "Joncs total length (both sides) ",x1*2.
...
c      Print joncs in BOX(1,7)
       ...
       if (k21d.eq.1) then
       xqw=0.
       xtext="List of nylon joncs"
       call txt(xpos+400.*xkf,ypos+(-700.+xqw)*
     + xkf,xmida1,0.0d0,xtext,7)
       do ng=1,ng21
       do i=ngo(ng,2),ngo(ng,3)
       xqw=xqw+15.
       write (xtext,'(A5,I3,2x,F7.1)') "Jonc ",i,rib(i,167)
       call txt(xpos+400*xkf,ypos+(-700.+xqw)*
...
c      write (*,*) "rn1= ", rn, rn1, rn2, rn3
     ...
     + ry0-(dx+0.5d0*requi)*dsin(ralp)+requi*dcos(ralp),7)
       dx=dx+requi
       call point(rx0+dx*dcos(ralp),ry0-dx*dsin(ralp),7)
       dx=dx+requi
...
c      Arc global coordinates (u,v) using 21 points
       do k=1,21
       ep=mu-(2.0d0*mu/20.0d0)*dfloat(k-1)
       parcu(k-1)=puntu(0)-(radi+xupp*0.1d0)*dsin(ep)
       parcv(k-1)=puntv(0)+(radi+xupp*0.1d0)*dcos(ep)
       end do
...
c      Trasnaltion using brute force (section 8.5.5) variable csi
...
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
c      Draw complete vent case ic2=1
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
...
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
c      Draw complete vent case ic2=-1 (intrados)
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
...
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
c      Draw complete vent case ic2=-4 (intrados)
...
c      Length vent rigth
     ...
     + (vfa(i,j,10)-vfa(i,j+1,10))**2.)
       ...
       xlenrr=xlenr*csi(i,20)

c      Interpolate new vent init
       do j=1,npo-1
       xlenl=xlenl+dsqrt((ufa(i,j,9)-ufa(i,j+1,9))**2.+
     + (vfa(i,j,9)-vfa(i,j+1,9))**2.)
       if (xlenl.ge.xlenrr) then
       npol=j+1
       end if
       end do
...
c      Print points jonc ini and fin BOX(1,2)
       call pointg(sepx+joini(i,1),sepy-joini(i,2),xcir,150)
       call pointg(sepx+jofin(i,1),sepy-jofin(i,2),xcir,150)

c      Laser cuting BOX(1,4)
       call point(sepx+joini(i,1)+xad4,sepy-joini(i,2),150)
       call point(sepx+jofin(i,1)+xad4,sepy-jofin(i,2),150)
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
c      Print final points vents, case print, vent +1
c      ic1=1 ic2=1
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
       if (ic1.eq.1.and.ic2.eq.1) then
c      Point left
       j=j9
       xu=ufr(i,j,9)
       xv=vfr(i,j,9)
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
c      Print final points vents, case laser, vent +1
c      ic1=2 ic2=1
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
       if (ic1.eq.2.and.ic2.eq.1) then
c      Point left
       j=j9
       xu=ufr(i,j,9)
       xv=vfr(i,j,9)
...
c      is=-1   Intrados
       ...
       call romano(i+1,psep+x1+u2,y1+v2+psey,alpha1,typm6(8)*0.1,4)
...
c      SUBROUTINE romanoparc(i,uu1,vv1,uu2,vv2,y3,psep,psey,xkf)
       ...
       integer npunt,j,iq,ic,is,kini,kfin
...
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
       ...
       call romano(i+1,psep+u2,psey-v2,alpha1,typm6(8)*0.1,4)
       call romano(i+1,psep+u2+2520.*xkf,psey-v2,alpha1,typm6(8)*0.1,4)
