...
class SqlTableModel(QSqlTableModel):
        ...
        if diff != 0:
            ...
            if mustNumConfigs > currNumConfigs:
                ...
                while i < diff:
                    ...
                    self.numRowsForConfigChanged.emit(currNumConfigs+1+i, 1)
                    i += 1
            else:
                # remove config lines
                while i < diff:
                    ...
                    # emit the change signal
                    self.numRowsForConfigChanged.emit(currNumConfigs-i, 0)
                    i += 1
    ...
    def removeRowForConfig(self, configNum):
        ...
        :param configNum: Number of the config for which the row must
                          be deleted.
        '''
    ...
    def setNumRowsForConfig(self, configNum, mustNumRows):
        ...
        if diff != 0:
            ...
            # emit the change signal
            self.numRowsForConfigChanged.emit(configNum,
                                              self.numRowsForConfig(configNum))

    def numRowsForConfig(self, configNum):
        '''
        :method: Use this to check how many rows a specific config
                 holds currently.
        :param configNum: Number of the config for which the number of rows
                          must be returned.
